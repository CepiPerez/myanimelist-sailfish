...
Page {
    ...
    Connections {
        ...
        onItemAdded: {
            myMangaLibrary = utils.manageList("add", myMangaLibrary, manga.id)
            var newmanga = newstatus + "<HSEP>" + manga.id + "<HSEP>" + manga.title + "<HSEP>"
                           + manga.url + "<HSEP>" + manga.synopsis + "<HSEP>" + manga.cover + "<HSEP>"
                           + manga.counter + "<HSEP>" + manga.started + "<HSEP>" + manga.finished + "<HSEP>"
                           + manga.score + "<HSEP>" +  manga.type + "<HSEP>" +  manga.itemtype
                           + "<HSEP>" +  manga.status

            manga = utils.updateItem(newmanga)
            removeMangaItem(manga)
            addMangaItem(manga)
        }
        onItemDeleted: {
            myMangaLibrary = utils.manageList("remove", myMangaLibrary, manga.id)
            removeMangaItem(manga)
            working = false
            pageStack.pop()
        }
        onDetailsDownloaded: {
            ...
            data.text = itemdata.type + "\n"
                        + qsTr("%1 volumes").arg(itemdata.counter) + "\n"
                        + itemdata.status
        }
    }
    ...
    onStatusChanged: {
        if (status === PageStatus.Activating) {
            if (manga.synopsis==="") {
                ...
                working = true
                api.getItemDetails("manga", manga.id)
            } else {
                working = false
            }
            ...
            inLybrary = utils.itemInLibrary(myMangaLibrary, manga.id)
        }
    }
    ...
    function processAction(action) {
        if (!inLybrary) {
            newstatus = action
            api.getAddItem("manga", manga.id, action)
        } else {
            if (action==="remove") {
                api.getDeleteItem("manga", manga.id)
            } else {
                newstatus = action
                api.getUpdateItem("manga", manga.id, action)
            }
        }
    }
    ...
    SilicaFlickable {
        ...
        //anchors.topMargin: currentTheme==="blanco"? 0 : toolBar.height
        ...
        Column {
            ...
            Image {
                ...
                smooth: true
                source: utils.getCacheImage(manga.cover)
            }
            ...
            Label {
                ...
                visible: !working
                text: manga.type + "\n"
                      + qsTr("%1 episodes").arg(manga.counter) + "\n"
                      + manga.status
            }
            ...
            Button {
                anchors.horizontalCenter: parent.horizontalCenter
                text: inLybrary? getMangaStatus(manga.mystatus) : qsTr("Add to Library")
                visible: !working
                onClicked: {
    ...
    ListModel {
        id: optionsModel
        ListElement{name: QT_TR_NOOP("Reading"); action:"1" }
        ...
        ListElement{name: QT_TR_NOOP("Plan to read"); action:"6" }
